...
template <typename PointT>
typename pcl::PointCloud<PointT>::Ptr makePntCld_like(const pcl::PointCloud<PointT>& other) {
    return makePntCld<PointT>();
}
template <typename PointT>
typename pcl::PointCloud<PointT>::Ptr makePntCld_randCube(const size_t pnt_count,
                                                          const float (&range)[2],
...
template <typename PointT>
typename pcl::PointCloud<PointT>::Ptr clone_from(pcl::PointCloud<PointT>& other) {
    auto res = makePntCld<PointT>();
...
template <typename PointT>
typename pcl::PointCloud<PointT>::Ptr ptcldFromFile(const std::string& file_path) {
...
typename pcl::visualization::PointCloudColorHandlerCustom<PointT>::Ptr single_color(
    pcl::PointCloud<PointT>& cld,
    const double (&color)[N]) {
...
template <typename PointT>
std::pair<float, float> axis_minmax(pcl::PointCloud<PointT>& ptcd, size_t axis) {
    auto& pts = ptcd.points;
    auto&& mm = std::minmax_element(pts.begin(), pts.end(), [axis](PointT& a, PointT& b) -> bool {
...
template <size_t N>
inline float* __numer_dot_addto(float numer, float (&input)[N], float* output = nullptr) {
    if (nullptr == output) {
        output = new float[N];
...
void __some_min_or_max_tail(Iter _b, Iter _e, size_t count, const bool is_min, Compare comp) {
...
std::shared_ptr<std::vector<std::vector<PointT>>> ___nn(Iter _b, Iter _e, size_t count) {
    std::vector<PointT> pts{_b, _e};
    std::shared_ptr<std::vector<std::vector<PointT>>> res{new std::vector<std::vector<PointT>>{}};

    while (pts.empty() == false) {
        some_min_tail(pts.begin(), pts.end(), count, [&](const PointT& a, const PointT& b) -> bool {
            return utils::euler_distance(pts.back(), a) < utils::euler_distance(pts.back(), b);
        });
        size_t cnt = count;
        res->push_back({});
...
template <typename PointT>
std::shared_ptr<std::vector<std::vector<PointT>>> ___nn(const std::vector<PointT>& _pts,
                                                        size_t count) {
...
typename pcl::PointCloud<PointT>::Ptr pass_filte(pcl::PointCloud<PointT>& cld,
                                                 ...
                                                 bool inRange = true,
                                                 bool located = false) {
    auto in_range = [](float v, float mi, float mx) -> bool { return mi <= v && v <= mx; };
    auto res      = makePntCld<PointT>();
    res->width    = cld.width;
